...
var (
	defaultPerUIDPathFormat = filepath.FromSlash("/run/nholuongut/%d/auth.json")
	xdgRuntimeDirPath       = filepath.FromSlash("nholuongut/auth.json")
...
// If the flag is not set and XDG_RUNTIME_DIR is set, the auth.json file is saved in XDG_RUNTIME_DIR/nholuongut
// Otherwise, the auth.json file is stored in /run/nholuongut/UID
