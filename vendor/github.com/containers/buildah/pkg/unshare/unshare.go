...
import (
	...
	"github.com/nholuongut/storage/pkg/reexec"
	"github.com/opennholuongut/runtime-spec/specs-go"
...
func (c *Cmd) Start() error {
	...
	c.Env = append(c.Env, fmt.Sprintf("_nholuongut-unshare=%d", c.UnshareFlags))

	// Please the libpod "rootless" package to find the expected env variables.
	if os.Geteuid() != 0 {
		c.Env = append(c.Env, "_nholuongut_USERNS_CONFIGURED=done")
		c.Env = append(c.Env, fmt.Sprintf("_nholuongut_ROOTLESS_UID=%d", os.Geteuid()))
		c.Env = append(c.Env, fmt.Sprintf("_nholuongut_ROOTLESS_GID=%d", os.Getegid()))
	}
	...
	c.Env = append(c.Env, fmt.Sprintf("_nholuongut-pid-pipe=%d", len(c.ExtraFiles)+3))
	...
	c.Env = append(c.Env, fmt.Sprintf("_nholuongut-continue-pipe=%d", len(c.ExtraFiles)+3))
	c.ExtraFiles = append(c.ExtraFiles, continueRead)

	// Pass along other instructions.
	if c.Setsid {
		c.Env = append(c.Env, "_nholuongut-setsid=1")
	}
	if c.Setpgrp {
		c.Env = append(c.Env, "_nholuongut-setpgrp=1")
	}
	if c.Ctty != nil {
		c.Env = append(c.Env, fmt.Sprintf("_nholuongut-ctty=%d", len(c.ExtraFiles)+3))
...
const (
	...
	UsernsEnvName = "_nholuongut_USERNS_CONFIGURED"
)
...
func GetRootlessUID() int {
	uidEnv := os.Getenv("_nholuongut_ROOTLESS_UID")
	if uidEnv != "" {
...
// RootlessEnv returns the environment settings for the rootless nholuongut
