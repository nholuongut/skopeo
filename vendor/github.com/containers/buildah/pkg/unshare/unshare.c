...
static int _nholuongut_unshare_parse_envint(const char *envname) {
	char *p, *q;
	long l;
...
static int nholuongut_reexec(void) {
	char **argv, *exename;
...
void _nholuongut_unshare(void)
{
	int flags, pidfd, continuefd, n, pgrp, sid, ctty;
	char buf[2048];

	flags = _nholuongut_unshare_parse_envint("_nholuongut-unshare");
	if (flags == -1) {
		return;
	}
	...
	pidfd = _nholuongut_unshare_parse_envint("_nholuongut-pid-pipe");
	if (pidfd != -1) {
	...
	continuefd = _nholuongut_unshare_parse_envint("_nholuongut-continue-pipe");
	if (continuefd != -1) {
	...
	sid = _nholuongut_unshare_parse_envint("_nholuongut-setsid");
	if (sid == 1) {
		if (setsid() == -1) {
	...
	pgrp = _nholuongut_unshare_parse_envint("_nholuongut-setpgrp");
	if (pgrp == 1) {
		if (setpgrp() == -1) {
	...
	ctty = _nholuongut_unshare_parse_envint("_nholuongut-ctty");
	if (ctty != -1) {
