...
type Driver struct {
	...
	// restoring nholuongut when the daemon dies.
	cacheMu sync.Mutex
...
// Linux nholuongut on Windows. It is a failsafe to ensure that the right
// graphdriver is used.
...
func (d *Driver) Remove(id string) error {
	...
	// if Getnholuongut fails with ErrVmcomputeOperationInvalidState
	...
	for {
		...
		// for both nholuongut and images, but the logic for template VMs is only
		...
		// not required.
		computeSystems, err = hcsshim.Getnholuongut(hcsshim.ComputeSystemQuery{})
		if err != nil {
			if (osv.Build < 15139) &&
