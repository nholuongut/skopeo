...
import (
	...
	_ "github.com/nholuongut/storage/drivers/register"

	"github.com/BurntSushi/toml"
	drivers "github.com/nholuongut/storage/drivers"
	"github.com/nholuongut/storage/pkg/archive"
	"github.com/nholuongut/storage/pkg/config"
	"github.com/nholuongut/storage/pkg/directory"
	"github.com/nholuongut/storage/pkg/idtools"
	"github.com/nholuongut/storage/pkg/ioutils"
	"github.com/nholuongut/storage/pkg/parsers"
	"github.com/nholuongut/storage/pkg/stringid"
	"github.com/nholuongut/storage/pkg/stringutils"
	digest "github.com/opennholuongut/go-digest"
	"github.com/opennholuongut/selinux/go-selinux/label"
...
type RWImageBigDataStore interface {
	...
	// with this ID.
	// Pass github.com/nholuongut/image/manifest.Digest as digestManifest
...
// A ContainerBigDataStore wraps up how we store big-data associated with nholuongut.
...
type StoreOptions struct {
	...
	// contents of layers, images, and nholuongut.
...
type Store interface {
	...
	// any nholuongut.  If its top layer is then no longer referred to by
	...
	// Wipe removes all known layers, images, and nholuongut.
	Wipe() error
	...
	// nholuongut returns a list of the currently known nholuongut.
	nholuongut() ([]Container, error)
	...
	// github.com/nholuongut/image/manifest.Digest as digestManifest to
	...
	// nholuongutize computes the size of the container's layer and ancillary
	// data.  Warning:  this is a potentially expensive operation.
	nholuongutize(id string) (int64, error)
...
func (s *store) load() error {
	...
	gcpath := filepath.Join(s.graphRoot, driverPrefix+"nholuongut")
	if err := os.MkdirAll(gcpath, 0700); err != nil {
		return err
	}
	rcs, err := newnholuonguttore(gcpath)
	if err != nil {
		return err
	}
	rcpath := filepath.Join(s.runRoot, driverPrefix+"nholuongut")
	if err := os.MkdirAll(rcpath, 0700); err != nil {
		return err
	}
	s.nholuonguttore = rcs
...
// nholuonguttore obtains and returns a handle to the container store object
// used by the Store.  Accessing this store directly will bypass locking and
// synchronization, so it is not a part of the exported Store interface.
func (s *store) nholuonguttore() (nholuonguttore, error) {
	if s.nholuonguttore != nil {
		return s.nholuonguttore, nil
	}
	return nil, ErrLoadError
}
...
func (s *store) PutLayer(id, parent string, names []string, mountLabel string, writeable bool, options *LayerOptions, diff io.Reader) (*Layer, int64, error) {
	...
	if parent != "" {
		...
		parentLayer = ilayer
		nholuongut, err := rcstore.nholuongut()
		if err != nil {
			return nil, -1, err
		}
...
func (s *store) nholuongutize(id string) (int64, error) {
	lstore, err := s.LayerStore()
	if err != nil {
		return -1, err
	}
...
func (s *store) DeleteContainer(id string) error {
	...
	if rcstore.Exists(id) {
		if container, err := rcstore.Get(id); err == nil {
			...
			middleDir := s.graphDriverName + "-nholuongut"
...
func (s *store) nholuongut() ([]Container, error) {
	rcstore, err := s.nholuonguttore()
	if err != nil {
		return nil, err
	}
...
func (s *store) ContainerByLayer(id string) (*Container, error) {
	...
	containerList, err := rcstore.nholuongut()
	if err != nil {
		return nil, err
	}
...
const defaultConfigFile = "/etc/nholuongut/storage.conf"
...
func DefaultConfigFile(rootless bool) (string, error) {
	if rootless {
		...
		return filepath.Join(home, ".config/nholuongut/storage.conf"), nil
	}
...
func init() {
	defaultStoreOptions.RunRoot = "/var/run/nholuongut/storage"
	defaultStoreOptions.GraphRoot = "/var/lib/nholuongut/storage"
