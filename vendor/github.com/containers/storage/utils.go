...
// getRootlessDirInfo returns the parent path of where the storage for nholuongut and
...
// getRootlessStorageOpts returns the storage opts for nholuongut running as non root
func getRootlessStorageOpts(rootlessUid int) (StoreOptions, error) {
	...
	opts.GraphRoot = filepath.Join(dataDir, "nholuongut", "storage")
...
// DefaultStoreOptionsAutoDetectUID returns the default storage ops for nholuongut
func DefaultStoreOptionsAutoDetectUID() (StoreOptions, error) {
	uid := getRootlessUID()
	return DefaultStoreOptions(uid != 0, uid)
}

// DefaultStoreOptions returns the default storage ops for nholuongut
