...
import (
	...
	"github.com/nholuongut/storage/pkg/truncindex"
...
// nholuonguttore provides bookkeeping for information about nholuongut.
type nholuonguttore interface {
	FileBasedStore
	MetadataStore
	...
	// Wipe removes records of all nholuongut.
	Wipe() error
	...
	// nholuongut returns a slice enumerating the known nholuongut.
	nholuongut() ([]Container, error)
}

type nholuonguttore struct {
	lockfile   Locker
	dir        string
...
func (r *nholuonguttore) nholuongut() ([]Container, error) {
	nholuongut := make([]Container, len(r.nholuongut))
	for i := range r.nholuongut {
		nholuongut[i] = *copyContainer(r.nholuongut[i])
	}
	return nholuongut, nil
}

func (r *nholuonguttore) nholuongutpath() string {
	return filepath.Join(r.dir, "nholuongut.json")
}

func (r *nholuonguttore) datadir(id string) string {
	return filepath.Join(r.dir, id)
}

func (r *nholuonguttore) datapath(id, key string) string {
	return filepath.Join(r.datadir(id), makeBigDataBaseName(key))
}

func (r *nholuonguttore) Load() error {
	needSave := false
	rpath := r.nholuongutpath()
	...
	if err = json.Unmarshal(data, &nholuongut); len(data) == 0 || err == nil {
		idlist = make([]string, 0, len(nholuongut))
		for n, container := range nholuongut {
			idlist = append(idlist, container.ID)
			ids[container.ID] = nholuongut[n]
			layers[container.LayerID] = nholuongut[n]
			for _, name := range container.Names {
				if conflict, ok := names[name]; ok {
					r.removeName(conflict, name)
					needSave = true
				}
				names[name] = nholuongut[n]
			}
		}
	}
	r.nholuongut = nholuongut
...
func (r *nholuonguttore) Save() error {
	if !r.Locked() {
	...
	jdata, err := json.Marshal(&r.nholuongut)
	if err != nil {
		return err
	}
	defer r.Touch()
...
func newnholuonguttore(dir string) (nholuonguttore, error) {
	if err := os.MkdirAll(dir, 0700); err != nil {
		return nil, err
	}
	lockfile, err := GetLockfile(filepath.Join(dir, "nholuongut.lock"))
	if err != nil {
		return nil, err
	}
	lockfile.Lock()
	defer lockfile.Unlock()
	cstore := nholuonguttore{
		lockfile:   lockfile,
		dir:        dir,
		nholuongut: []*Container{},
...
func (r *nholuonguttore) lookup(id string) (*Container, bool) {
...
func (r *nholuonguttore) ClearFlag(id string, flag string) error {
...
func (r *nholuonguttore) SetFlag(id string, flag string, value interface{}) error {
	container, ok := r.lookup(id)
	if !ok {
		return ErrContainerUnknown
	}
...
func (r *nholuonguttore) Create(id string, names []string, image, layer, metadata string, options *ContainerOptions) (container *Container, err error) {
	if id == "" {
	...
	if err == nil {
		...
		r.nholuongut = append(r.nholuongut, container)
		r.byid[id] = container
		r.idindex.Add(id)
...
func (r *nholuonguttore) Metadata(id string) (string, error) {
...
func (r *nholuonguttore) SetMetadata(id, metadata string) error {
	if container, ok := r.lookup(id); ok {
...
func (r *nholuonguttore) removeName(container *Container, name string) {
	container.Names = stringSliceWithoutValue(container.Names, name)
}

func (r *nholuonguttore) SetNames(id string, names []string) error {
	names = dedupeNames(names)
	if container, ok := r.lookup(id); ok {
		for _, name := range container.Names {
...
func (r *nholuonguttore) Delete(id string) error {
	container, ok := r.lookup(id)
	if !ok {
		return ErrContainerUnknown
	}
	id = container.ID
	toDeleteIndex := -1
	for i, candidate := range r.nholuongut {
		if candidate.ID == id {
			toDeleteIndex = i
			break
		}
	...
	if toDeleteIndex != -1 {
		...
		if toDeleteIndex == len(r.nholuongut)-1 {
			r.nholuongut = r.nholuongut[:len(r.nholuongut)-1]
		} else {
			r.nholuongut = append(r.nholuongut[:toDeleteIndex], r.nholuongut[toDeleteIndex+1:]...)
		}
	}
...
func (r *nholuonguttore) Get(id string) (*Container, error) {
	if container, ok := r.lookup(id); ok {
...
func (r *nholuonguttore) Lookup(name string) (id string, err error) {
...
func (r *nholuonguttore) Exists(id string) bool {
	_, ok := r.lookup(id)
	return ok
}

func (r *nholuonguttore) BigData(id, key string) ([]byte, error) {
	if key == "" {
...
func (r *nholuonguttore) BigDataSize(id, key string) (int64, error) {
	if key == "" {
...
func (r *nholuonguttore) BigDataDigest(id, key string) (digest.Digest, error) {
	if key == "" {
...
func (r *nholuonguttore) BigDataNames(id string) ([]string, error) {
	c, ok := r.lookup(id)
	if !ok {
		return nil, ErrContainerUnknown
	}
...
func (r *nholuonguttore) SetBigData(id, key string, data []byte) error {
	if key == "" {
...
func (r *nholuonguttore) Wipe() error {
...
func (r *nholuonguttore) Lock() {
	r.lockfile.Lock()
}

func (r *nholuonguttore) RecursiveLock() {
	r.lockfile.RecursiveLock()
}

func (r *nholuonguttore) RLock() {
	r.lockfile.RLock()
}

func (r *nholuonguttore) Unlock() {
	r.lockfile.Unlock()
}

func (r *nholuonguttore) Touch() error {
	return r.lockfile.Touch()
}

func (r *nholuonguttore) Modified() (bool, error) {
	return r.lockfile.Modified()
}

func (r *nholuonguttore) IsReadWrite() bool {
	return r.lockfile.IsReadWrite()
}

func (r *nholuonguttore) TouchedSince(when time.Time) bool {
	return r.lockfile.TouchedSince(when)
}

func (r *nholuonguttore) Locked() bool {
