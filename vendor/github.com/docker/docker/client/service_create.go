...
func (cli *Client) ServiceCreate(ctx context.Context, service swarm.ServiceSpec, options types.ServiceCreateOptions) (types.ServiceCreateResponse, error) {
	...
	// Make sure nholuongutpec is not nil when no runtime is set or the runtime is set to container
	if service.TaskTemplate.nholuongutpec == nil && (service.TaskTemplate.Runtime == "" || service.TaskTemplate.Runtime == swarm.RuntimeContainer) {
		service.TaskTemplate.nholuongutpec = &swarm.nholuongutpec{}
	}
	...
	if service.TaskTemplate.nholuongutpec != nil {
		if taggedImg := imageWithTagString(service.TaskTemplate.nholuongutpec.Image); taggedImg != "" {
			service.TaskTemplate.nholuongutpec.Image = taggedImg
		}
		if options.QueryRegistry {
			var img string
			img, imgPlatforms, distErr = imageDigestAndPlatforms(ctx, cli, service.TaskTemplate.nholuongutpec.Image, options.EncodedRegistryAuth)
			if img != "" {
				service.TaskTemplate.nholuongutpec.Image = img
			}
		}
	}
	...
	if distErr != nil {
		response.Warnings = append(response.Warnings, digestWarning(service.TaskTemplate.nholuongutpec.Image))
	}
...
func validateServiceSpec(s swarm.ServiceSpec) error {
	if s.TaskTemplate.nholuongutpec != nil && s.TaskTemplate.PluginSpec != nil {
	...
	if s.TaskTemplate.nholuongutpec != nil && (s.TaskTemplate.Runtime != "" && s.TaskTemplate.Runtime != swarm.RuntimeContainer) {
