...
// currentnholuonguttarts is used to limit the number of concurrent container
// starts.
var currentnholuonguttarts nholuonguttarts

type nholuonguttarts struct {
	maxParallel int
	inProgress  int
	sync.Mutex
}
...
func init() {
	...
	if len(mpsS) > 0 {
		...
		currentnholuonguttarts.maxParallel = mpsI
	}
}
...
func (computeSystem *System) Start() (err error) {
	...
	if currentnholuonguttarts.maxParallel > 0 {
		for {
			currentnholuonguttarts.Lock()
			if currentnholuonguttarts.inProgress < currentnholuonguttarts.maxParallel {
				currentnholuonguttarts.inProgress++
				currentnholuonguttarts.Unlock()
				break
			}
			if currentnholuonguttarts.inProgress == currentnholuonguttarts.maxParallel {
				currentnholuonguttarts.Unlock()
