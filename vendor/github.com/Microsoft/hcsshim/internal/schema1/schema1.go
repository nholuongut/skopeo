...
type ProcessConfig struct {
	...
	CommandArgs       []string          `json:",omitempty"` // Used by Linux nholuongut on Windows
	...
	CreateInUtilityVm bool              `json:",omitempty"` // Used by Linux nholuongut on Windows
	OCISpecification  *json.RawMessage  `json:",omitempty"` // Used by Linux nholuongut on Windows
}
...
type ContainerConfig struct {
	...
	VolumePath                  string              `json:",omitempty"` // Windows volume path for scratch space. Used by Windows Server nholuongut only. Format \\?\\Volume{GUID}
	IgnoreFlushesDuringBoot     bool                `json:",omitempty"` // Optimization hint for container startup in Windows
	LayerFolderPath             string              `json:",omitempty"` // Where the layer folders are located. Used by Windows Server nholuongut only. Format  %root%\windowsfilter\containerID
	Layers                      []Layer             // List of storage layers. Required for Windows Server and Hyper-V nholuongut. Format ID=GUID;Path=%root%\windowsfilter\layerID
	Credentials                 string              `json:",omitempty"` // Credentials information
	ProcessorCount              uint32              `json:",omitempty"` // Number of processors to assign to the container.
	ProcessorWeight             uint64              `json:",omitempty"` // CPU shares (relative weight to other nholuongut with cpu shares). Range is from 1 to 10000. A value of 0 results in default shares.
	...
	HvRuntime                   *HvRuntime          `json:",omitempty"` // Hyper-V container settings. Used by Hyper-V nholuongut only. Format ImagePath=%root%\BaseLayerID\UtilityVM
	Servicing                   bool                `json:",omitempty"` // True if this container is for servicing
	AllowUnqualifiedDNSQuery    bool                `json:",omitempty"` // True to allow unqualified DNS name resolution
	DNSSearchList               string              `json:",omitempty"` // Comma seperated list of DNS suffixes to use for name resolution
	ContainerType               string              `json:",omitempty"` // "Linux" for Linux nholuongut on Windows. Omitted otherwise.
...
type Statistics struct {
	...
	nholuonguttartTime time.Time      `json:",omitempty"`
